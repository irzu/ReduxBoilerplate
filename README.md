...
Setup differs from previous chapters as we have two separate apps now.
Since we won't be introducing a tool like `lerna` to run them simultanousely
we'll need to duplicate the steps and launch the apps from two different terminal windows

In first window we'll run the `countriesApp` on port 3005
```
cd countriesApp
npm install
npm start
```
In second window we'll start the `provincesApp` on port 3006
```
cd provincesApp
npm install
npm start
